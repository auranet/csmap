/******************************************************************************
	...
	*********************************************************************
			 ...
			 File Name: $RCSfile: CS_rlsUpdt.c $
		   Description:
			   Purpose:
		...
		Last Revision:	$Log: CS_rlsUpdt.c $
		...
		Last Revision:	Revision 11.601  2003/12/19 01:09:08  nto
		...
		Last Revision:	Revision 11.101  2003/02/15 01:29:54  nto
		Last Revision:	Added eof checks in three places where they were ommitted in the ANSI conversion effort.
		Last Revision:	Revision 11.100  2002/11/22 19:38:47  nto
		Last Revision:	Official release: 11.01
		Last Revision:	Revision 10.1505  2002/11/21 21:08:24  nto
		Last Revision:	Lint cleanup.
		Last Revision:	Revision 10.1504  2002/11/06 23:53:10  nto
		Last Revision:	Adjusted to compile under various flavors of UNIX.
		Last Revision:	Revision 10.1503  2002/10/06 22:24:54  nto
		Last Revision:	Fixed ANSI problems related to unlink, remove, and rename.
		Last Revision:	Revision 10.1502  2002/10/06 18:07:04  nto
		Last Revision:	Converted all direct references to 'access' to CS_access.
		Last Revision:	Revision 10.1501  2002/10/06 17:52:48  nto
		Last Revision:	Major surgery to amke ANSI compliant.  Much of this remains untested as of this date.
		...
		Last Revision:	Revision 10.700  2001/01/25 04:13:53  nto
		Last Revision:	Revision 10.600  2000/12/11 06:39:48  nto
		Last Revision:	Release 10.06
		Last Revision:	Revision 10.501  2000/12/11 05:32:19  nto
		Last Revision:	Adjusted for the new Multiple Regression structure.
		Last Revision:	Revision 10.500  2000/10/27 07:38:25  nto
		Last Revision:	Official Release 10.05
		Last Revision:	Revision 10.400  2000/09/13 01:16:59  nto
		Last Revision:	Release 10.04
		Last Revision:	Revision 10.300  2000/04/04 23:18:30  nto
		Last Revision:	Revision 10.200  2000/03/02 18:51:21  nto
		Last Revision:	Revision 10.2  1999/11/03 23:41:07  nto
		Last Revision:	Results of datum conversion rewrite.
		Last Revision:	Revision 10.1  1999/10/09 00:18:58  nto
		...
		Last Revision:	Revision 1.2  1999/09/07 15:18:14  nto
		Last Revision:	This revision compiles, links, and generates a successful test program.

******************************************************************************/

#ifdef LABEL_MODULE
static char revlvl [] = {"$Id: //depot/Libraries/CsMap/current/src/CS_rlsUpdt.c#10 $"};
#endif
		...
		if (sp->temp_nm [0] != '\0')
		{
			...
			*cs_DirP = '\0';
			...
			if (st == 0)
			{
				...
				CS_rename (tmp_name,cs_Dir);					/*lint !e534 */
			}
			else
			{
				remove (tmp_name);
...
int EXP_LVL9 CSswpatl (const char *fname,const struct _stat *stat_ptr,int parm)
{
	...
	/* If the given file is not an ordinary file, we ignore the
	   entry. */
	...
	/* Verify that this file came from the specific directory
	   we were told to search.  This is necessary only for
	   UNIX systems as the DOS findfirst/findnext functions
	   do not recurse sub-directories.  But we do it anyway
	   to check this code. */
	...
	/* If cp is NULL at this point, there is no directory
	   separator, therefore we must still be in the initial
	   diirectory in which the seacrh started.  Therefore,
	   we should be OK.  This should never happen anyway,
	   unless the user has set cs_Dir to the null string
	   attempting to access the current working directory. */

	if (cp != NULL)
	{
		/* Terminate the directory portion of the
		   path name we have been given.  Notice that
		   we place this null immediately after the
		   directory termination character so that
		   our result should match cs_Dir exactly. */

		*(cp + 1) = '\0';

		/* See if we are still in the directory indicated
		   by cs_Dir. */
	...
	/* If there is no extension, is cannot be a file of
	   interest to us. */
	...
	/* If we have an extension that we recognize, add the
...
char * EXP_LVL7 CS_swpfl (Const char org_name [])
{
	...
	*cs_DirP = '\0';
	...
	cp = strrchr (org_name,cs_ExtsepC);
	...
	else if (cp != NULL && (!CS_stricmp ((cp + 1),cs_NADCON_LOS) ||
							!CS_stricmp ((cp + 1),cs_NADCON_LAS)
						   )
			)
	{
		/* Here for a NADCON file.  We have to read the header in
		   order to get the size of the individual records.  We
		   might even have to swap the ele_cnt to get the right
		   number of records.  That is, the files we are swapping
		   may be formatted for the machine we are swapping from. */
		...
		hdr_size = rec_size = (int)((nad_hdr.ele_cnt * sizeof (float)) + sizeof (long));	/*lint !e737 */

		res_size = 0;
		res_frmt [0] = '\0';
	}
	else if (cp != NULL && !CS_stricmp ((cp + 1),cs_MULREG_EXT))
	{
		/* Here for a Multiple Regression file.  This is
		   different from all the others.  We just read
		   it in, swap it, and write it back out.  The
		   following turns off the general conversion
		   used below for all other files. */
		...
		/* See if this file is in the right byte order for
		   our current machine.  If not, we need to swap
		   the file size element of the structure. */
		...
		wrCnt = CS_fwrite (&grfmreg,1,my_size,outStrm);
		if (wrCnt != (unsigned)my_size)
		{
	...
	else if (cp != NULL && !CS_stricmp ((cp + 1),cs_CANNT_DAC))
	{
		/* Here for a Version 1 Canadian National Transformation
		   file.  Very similar to a NADCON file, we'll skip the
		   comments. */
		...
		/* The Canadians use a delta of 5 minutes.  We assume the
		   record needs to be swapped if the delta is not equal to
		...
		/* Canadian files contain doubles, and don't have the
		   record ID that US files have. */
		sprintf (rec_frmt,"%dd",ele_cnt * 2);
		hdr_size = sizeof (cannt_hdr);
		rec_size = 2 * ele_cnt * sizeof (double);		/*lint !e737 !e713 */
	...
	*/
	...
	if (rec_size != 0 || hdr_size != 0)
	{
		...
		if (hdr_size != 0)
		{
			bufr = CS_malc (hdr_size);				/*lint !e732 */
			if (bufr == NULL)
			{
				CS_erpt (cs_NO_MEM);
				goto error;
			...
			wrCnt = CS_fwrite (bufr,1,hdr_size,outStrm);	/*lint !e732 */
		...
		if (rec_size != 0)
		{
			bufr = CS_malc (rec_size);				/*lint !e732 */
			if (bufr == NULL)
			{
				CS_erpt (cs_NO_MEM);
				goto error;
			}
			while ((int)(rdCnt = CS_fread (bufr,1,rec_size,inStrm)) == rec_size)		/*lint !e732 */
			{
				CS_bswap (bufr,rec_frmt);
				wrCnt = CS_fwrite (bufr,1,rdCnt,outStrm);						/*lint !e732 */
				if (wrCnt != rdCnt)
				{
					CS_erpt (CS_ferror (outStrm) ? cs_IOERR : cs_DISK_FULL);
					goto error;
...
error:
	if (inStrm != NULL) CS_fclose (inStrm);						/*lint !e644 */
	if (outStrm != NULL)
	{
		CS_fclose (outStrm);
		remove (out_name);
...
static int CScsrupReadOld (csFILE *oldStrm,struct csCsrup_ *csrup,int old_lvl);
static int CScsrupRead05 (csFILE *oldStrm,struct csCsrup_ *csrup);
static int CScsrupRead06 (csFILE *oldStrm,struct csCsrup_ *csrup);
static int CScsrupRead07 (csFILE *oldStrm,struct csCsrup_ *csrup);
static int CScsrupRead08 (csFILE *oldStrm,struct csCsrup_ *csrup);
static int CScsrupTo06 (struct csCsrup_ *csrup);
static int CScsrupTo07 (struct csCsrup_ *csrup);
static int CScsrupTo08 (struct csCsrup_ *csrup);
...
int EXP_LVL3 CS_csrup (Const char *distrb,Const char *bkupnm)
{
	...
	if (distrb != NULL && distrb [0] != '\0')
	{
		...
		if (rdCnt != sizeof (magic))					/*lint !e737 */
		{
	...
	else if (updStrm == NULL && oldStrm != NULL)
	{
		...
		while (TRUE)
		{
			old_st = CScsrupReadOld (oldStrm,&csCsrup,old_lvl);			/*lint !e644 */
			if (old_st < 0) goto error;
			if (old_st == 0) break;
			CS_cswr (tmpStrm,&csCsrup.u.csdefCUR,csCsrup.crypt);			/*lint !e534 */
		}
	}
	else
	{
		...
		while (upd_st > 0 || old_st > 0)
		{
			...
			if (cmp == 0)
			{
				/* Write the distribution record, can the
				   the old record. */
				CS_cswr (tmpStrm,&cs_Csdef,upd_crypt);					/*lint !e534 */
				upd_st = CS_csrd (updStrm,&cs_Csdef,&upd_crypt);
				if (upd_st < 0) goto error;
				old_st = CScsrupReadOld (oldStrm,&csCsrup,old_lvl);
				if (old_st < 0) goto error;
	...
	if (bkupnm != NULL && *bkupnm != '\0')
	{
		...
		CS_rename (cs_Dir,ctemp);								/*lint !e534 */
...
int CScsrupRead05 (csFILE *oldStrm,struct csCsrup_ *csrup)
{
	...
	rdCnt = CS_fread (ptr,1,sizeof (*ptr),oldStrm);
	if (rdCnt != sizeof (*ptr))
	{
...
double EXP_LVL9 CScsrupPNT (struct cs_Csdef_ *csdef)
{
	...
	Az = CS_llazdd (e_rad,ecent*ecent,ll0,ll1,&dummy);		/*lint !e644 */
	return (Az);
}
...
static int CSdtrupReadOld (csFILE *oldStrm,struct csDtrup_ *dtrup,int old_lvl);
static int CSdtrupRead05 (csFILE *oldStrm,struct csDtrup_ *dtrup);
static int CSdtrupRead06 (csFILE *oldStrm,struct csDtrup_ *dtrup);
static int CSdtrupRead07 (csFILE *oldStrm,struct csDtrup_ *dtrup);
static int CSdtrupRead08 (csFILE *oldStrm,struct csDtrup_ *dtrup);
static int CSdtrupTo06 (struct csDtrup_ *dtrup);
static int CSdtrupTo07 (struct csDtrup_ *dtrup);
static int CSdtrupTo08 (struct csDtrup_ *dtrup);
...
int EXP_LVL3 CS_dtrup (Const char *distrb,Const char *bkupnm)
{
	...
	else if (updStrm == NULL && oldStrm != NULL)
	{
		...
		while (TRUE)
		{
			old_st = CSdtrupReadOld (oldStrm,&csDtrup,old_lvl);	/*lint !e644 */
			if (old_st < 0) goto error;
			if (old_st == 0) break;
			CS_dtwr (tmpStrm,&csDtrup.u.dtdefCUR,csDtrup.crypt);	/*lint !e534 */
		}
	}
	else
	{
		...
		while (upd_st > 0 || old_st > 0)
		{
			...
			if (cmp == 0)
			{
				/* Write the distribution record, can the
				   the old record. */
				CS_dtwr (tmpStrm,&cs_Dtdef,upd_crypt);			/*lint !e534 */
				upd_st = CS_dtrd (updStrm,&cs_Dtdef,&upd_crypt);
				if (upd_st < 0) goto error;
				old_st = CSdtrupReadOld (oldStrm,&csDtrup,old_lvl);
				if (old_st < 0) goto error;
...
static int CSelrupReadOld (csFILE *oldStrm,struct csElrup_ *elrup,int old_lvl);
static int CSelrupRead05 (csFILE *oldStrm,struct csElrup_ *elrup);
static int CSelrupRead07 (csFILE *oldStrm,struct csElrup_ *elrup);
static int CSelrupRead08 (csFILE *oldStrm,struct csElrup_ *elrup);
static int CSelrupTo07 (struct csElrup_ *elrup);
static int CSelrupTo08 (struct csElrup_ *elrup);
...
int EXP_LVL3 CS_elrup (Const char *distrb,Const char *bkupnm)
{
	...
	else if (updStrm == NULL && oldStrm != NULL)
	{
		...
		while (TRUE)
		{
			old_st = CSelrupReadOld (oldStrm,&csElrup,old_lvl);		/*lint !e644 */
			if (old_st < 0) goto error;
			if (old_st == 0) break;
			CS_elwr (tmpStrm,&csElrup.u.eldefCUR,csElrup.crypt);		/*lint !e534 */
		}
	}
	else
	{
		...
		while (upd_st > 0 || old_st > 0)
		{
			...
			if (cmp == 0)
			{
				/* Write the distribution record, can the
				   the old record. */
				CS_elwr (tmpStrm,&cs_Eldef,upd_crypt);		/*lint !e534 */
				upd_st = CS_elrd (updStrm,&cs_Eldef,&upd_crypt);
				if (upd_st < 0) goto error;
				old_st = CSelrupReadOld (oldStrm,&csElrup,old_lvl);
				if (old_st < 0) goto error;
