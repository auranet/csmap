/******************************************************************************
	...
	*********************************************************************  
		...
		Last Revision:	Revision 10.301  2000/09/12 21:03:50  nto
		Last Revision:	FIxed cut & paste bug in the extents calculation.
		Last Revision:	Revision 10.300  2000/04/04 23:18:29  nto
		Last Revision:	Revision 10.200  2000/03/02 18:51:21  nto
		Last Revision:	Revision 10.1  2000/01/30 23:59:57  nto
		...
		Last Revision:	Revision 1.2  1999/09/05 19:26:36  nto
		Last Revision:	This revision compiles, links, and generates a successful test program.

******************************************************************************/

#ifdef LABEL_MODULE
static char revlvl [] = {"$Id: //depot/Libraries/CsMap/current/src/CS_ostro.c#3 $"};
#endif
...
int EXP_LVL9 CSostroI (Const struct cs_Ostro_ *ostro,double ll [2],Const double xy [2])
{
	...
	if (fabs (ll [LAT]) > cs_NPTest && rtn_val == cs_CNVRT_NRML)
	{
		rtn_val = cs_CNVRT_INDF;
	}
	ll [LNG] *= cs_Radian;
