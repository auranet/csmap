/******************************************************************************
	...
	*********************************************************************  
			 ...
			 File Name: $RCSfile: CS_optional.c $
		   Description:
			   Purpose:
		...
		Last Revision:	$Log: CS_optional.c $
		...
		Last Revision:	Revision 11.901  2004/08/28 23:30:32  nto
		Last Revision:	Fixed WY-W (4904)
		...
		Last Revision:	Revision 10.701  2001/09/18 19:30:45  nto
		Last Revision:	Replaced ecvt with CS_ecvt.  ecvt is not ANSI standard.
		...
		Last Revision:	Revision 10.302  2000/09/13 00:45:15  nto
		Last Revision:	Fixed a problem with leading zeros when seconds are 10.0000.
		Last Revision:	Revision 10.301  2000/07/15 22:55:25  nto
		Last Revision:	Fixed a Typo on the Massachussetts Island coordinate system
		Last Revision:	Revision 10.300  2000/04/04 23:18:29  nto
		Last Revision:	Revision 10.201  2000/04/03 21:42:11  nto
		Last Revision:	Fixed the longitude range detection on input.
		Last Revision:	Revision 10.200  2000/03/02 18:51:21  nto
		Last Revision:	Revision 10.1  2000/01/30 23:58:11  nto
		Last Revision:	Added the range format feature.  Also some PCLint clean up.
		Last Revision:	Revision 10.0  1999/10/05 18:27:17  nto
		Last Revision:	Revision 9.99  1999/10/05 17:36:28  nto
		Last Revision:	Revision 1.2  1999/09/06 20:01:22  nto
		Last Revision:	This revision compiles, links, and generates a successful test program.

******************************************************************************/

#ifdef LABEL_MODULE
static char revlvl [] = {"$Id: //depot/Libraries/CsMap/current/src/CS_optional.c#3 $"};
#endif
...
**	long flags;					returns information about the format processed,
...

{
	...
	while (*ccp != '\0' && (*ccp < '0' || *ccp > '9') && *ccp != cs_DecpntC)
	{
		if (*ccp == '-' || *ccp == '+')
		{
		        sgn_flg = TRUE;
	...
	while (tot_cnt > 0 && (cc = *ccp++) != '\0')
	{
		...
		if (cc != cs_DecpntC && cc != cs_DigsepC &&
		    (cc < '0' || cc > '9'))
		{
...
**	long frmt;					a bit map of options according to the
**								format rertuned by CS_atof.
**	long actual;				returns the actual format used to
**								output the value.
**********************************************************************/
