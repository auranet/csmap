/******************************************************************************
	...
	*********************************************************************  
		...
		Last Revision:	Revision 10.1502  2002/11/06 23:51:39  nto
		Last Revision:	Removed byte swapping code; it's not necessary.
		...
		Last Revision:	Revision 1.1  2002/02/19 05:44:23  nto
		Last Revision:	Initial Revision

******************************************************************************/

#ifdef LABEL_MODULE
static char revlvl [] = {"$Id: //depot/Libraries/CsMap/current/src/CS_ostn97.c#5 $"};
#endif
...
int CSmkBinaryOstn97 (struct cs_Ostn97_ *__This)
{
	...
	if (bTime == 0 || bTime <= aTime)
	{
		...
		while (CS_fgets (lineBufr,sizeof (lineBufr),aStrm) != NULL)
		{
			...
			*cp++ = '\0';
			floatBufr [0] = (float)atof (lineBufr);
			floatBufr [1] = (float)atof (cp);
