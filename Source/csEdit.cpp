/******************************************************************************
	...
	*********************************************************************  
		...
		Last Revision:	Revision 11.601  2003/12/19 01:02:58  nto
		Last Revision:	Adjusted MFC code to compile with VC++ 7.0
		...
		Last Revision:	Revision 11.301  2003/04/21 23:21:05  nto
		Last Revision:	Increase the precision on azimuth parameters.
		Last Revision:	Revision 11.300  2003/02/25 04:29:13  nto
		Last Revision:	Official Release 11.03
		Last Revision:	Revision 11.101  2003/02/21 06:31:54  nto
		Last Revision:	Added a confirmation to a delete request.
		Last Revision:	Revision 11.100  2002/11/22 19:38:40  nto
		Last Revision:	Official release: 11.01
		Last Revision:	Revision 10.1501  2002/10/15 04:53:06  nto
		...
		Last Revision:	Revision 10.901  2002/01/30 01:01:11  nto
		Last Revision:	Changed the lat/long format default to include 4 digits of seconds.
		Last Revision:	Revision 10.900  2001/09/24 04:31:08  nto
		Last Revision:	Revision 10.701  2001/09/18 19:06:16  nto
		Last Revision:	Fixed error handling when dictionaries not present.  Also replaced
		Last Revision:	strnicmp with CS_strnicmp.  strnicmp is not ANSI standard.
		...
		Last Revision:	Revision 10.301  2000/09/12 21:29:58  nto
		Last Revision:	Added the new Affine Coefficient parameter type.
...
#ifdef LABEL_MODULE
static char revlvl [] = {"$Id: //depot/Libraries/CsMap/current/src/csEdit.cpp#7 $"};
#endif
...
void EXP_LVL1 CS_csEditor (char *csKeyName)
{
	CcsEdit csEditor;
	CS_recvr ();
	csEditor.SetKeyName (csKeyName);
	csEditor.DoModal ();
	CS_stncp (csKeyName,csEditor.GetKeyName (),cs_KEYNM_DEF);
	return;
}
/////////////////////////////////////////////////////////////////////////////
// CcsEdit
CcsEdit::CcsEdit(CWnd* pParentWnd, UINT iSelectPage)
	:CPropertySheet("Coordinate System Editor", pParentWnd, iSelectPage)
{
...
void CcsEdit::SetKeyName (const char* csKeyName)
{
	char kyTemp [cs_KEYNM_DEF + 2];

	CS_stncp (kyTemp,csKeyName,sizeof (kyTemp));
	CS_nampp (kyTemp);
...
const char* CcsEdit::GetKeyName ()
{
	return (LPCTSTR)m_KeyName;
}
void CcsEdit::SetLongitudeFormat (const long format)
{
	m_LngFrmt = format;
	AdjustParmArray ();
}
void CcsEdit::SetLatitudeFormat (const long format)
{
	m_LatFrmt = format;
	AdjustParmArray ();
}
void CcsEdit::SetLatLongFormat (const long format)
{
	m_LngFrmt = format | cs_ATOF_XEAST;
	m_LatFrmt = format & (~((long)cs_ATOF_XEAST));
	AdjustParmArray ();
}
void CcsEdit::SetXFormat (const long format)
{
	m_XFrmt = format;
	AdjustParmArray ();
}
void CcsEdit::SetYFormat (const long format)
{
	m_YFrmt = format;
	AdjustParmArray ();
}
void CcsEdit::SetXYFormat (const long format)
{
	m_XFrmt = format | cs_ATOF_XEAST;
	m_YFrmt = format & (~((long)cs_ATOF_XEAST));
	AdjustParmArray ();
}
BOOL CcsEdit::OnInitDialog (void)
{
	...
	wp = GetDlgItem (IDOK);
	if (wp != NULL) wp->SetWindowText ("Close");
	wp = GetDlgItem (IDCANCEL);
	if (wp != NULL) wp->SetWindowText ("New");
	wp = GetDlgItem (ID_APPLY_NOW);
	if (wp != NULL)	wp->SetWindowText ("Delete");
...
void CcsEdit::DisperseData (struct cs_Csdef_ *csPtr)
{
	...
	m_KeyName = csPtr->key_nm;
	m_PgIdent.m_Description = csPtr->desc_nm;
	m_Description = csPtr->desc_nm;
	m_PgIdent.m_Source = csPtr->source;
	m_PgIdent.m_Group = csPtr->group;
	m_PgIdent.m_CountrySt = csPtr->cntry_st;
	m_PgIdent.m_Location = csPtr->locatn;

	m_PgGnrl.m_PrjKeyName = csPtr->prj_knm;
	if (csPtr->dat_knm [0] != '\0')
	{
		m_PgGnrl.m_RefType = "Datum";
		m_PgGnrl.m_RefKeyName = csPtr->dat_knm;
		cp = csPtr->dat_knm;
	...
	{
		m_PgGnrl.m_RefType = "Ellipsoid";
		m_PgGnrl.m_RefKeyName = csPtr->elp_knm;
		cp = csPtr->elp_knm;
		len = static_cast<int>(strlen (cp));
		cpl = cp + len - 1;
		m_PgGnrl.m_RefToDflt = (*cp == cs_DFLT_IDNTBEG && *cpl == cs_DFLT_IDNTEND);
...
void CcsEdit::CollectData (struct cs_Csdef_ *csPtr)
{
	...
	CS_stncp (csPtr->key_nm,m_KeyName,sizeof (csPtr->key_nm));
	CS_stncp (csPtr->desc_nm,m_PgIdent.m_Description,sizeof (csPtr->desc_nm));
	CS_stncp (csPtr->source,m_PgIdent.m_Source,sizeof (csPtr->source));
	CS_stncp (csPtr->group,m_PgIdent.m_Group,sizeof (csPtr->group));
	CS_stncp (csPtr->cntry_st,m_PgIdent.m_CountrySt,sizeof (csPtr->cntry_st));
	CS_stncp (csPtr->locatn,m_PgIdent.m_Location,sizeof (csPtr->locatn));

	CS_stncp (csPtr->prj_knm,m_PgGnrl.m_PrjKeyName,sizeof (csPtr->prj_knm));
	cp = ctemp;
	if (m_PgGnrl.m_RefToDflt) *cp++ = cs_DFLT_IDNTBEG;
	cp = CS_stncp (cp,m_PgGnrl.m_RefKeyName,sizeof (ctemp) - 2);
	if (m_PgGnrl.m_RefToDflt) *cp++ = cs_DFLT_IDNTEND;
	*cp = '\0';
	if (m_PgGnrl.m_RefType [0] == 'D')
	{
		csPtr->elp_knm [0] = '\0';
	...
	cp = CS_stncp (cp,m_PgGnrl.m_Unit,sizeof (ctemp) - 2);
...
//	unchanged.
bool CcsEdit::GetCoordSys (const char *keyName)
{
	struct cs_Csdef_ *cs_def;

	// Get the definition to be displayed.
	cs_def = CS_csdef (keyName);
	if (cs_def == NULL)
	{
		char errMesg [256];
...
bool CcsEdit::ChangeCoordSys (const char *keyName)
{
...
void CcsEdit::NewCoordSys (const CString& csKeyName)
{
	...
	CS_stncp (csDef.key_nm,(LPCTSTR)csKeyName,sizeof (csDef.key_nm));
...
bool CcsEdit::DeleteCoordSys (const CString& keyName)
{
	...
	if (m_Protected)
	{
		AfxMessageBox ("Displayed coordinate system definition is protected and may not be deleted.");
		st = IDCANCEL;
		rtnValue = false;
	}
	else
	{
		st = AfxMessageBox ("Click 'OK' to confirm your request to delete this coordinate system definition.",MB_OKCANCEL | MB_ICONQUESTION | MB_DEFBUTTON2);
		rtnValue = false;
	}
	if (st == IDOK)
	{
		memset (&csDef,'\0',sizeof (csDef));
		CS_stncp (csDef.key_nm,m_KeyName,sizeof (csDef.key_nm));
...
void CcsEdit::OnCsEdNewCncl() 
{
	...
	{	// Here to create a new coordinate system.  Save the current name for
		...
		// operator provide the name of the new datum.
...
// the property page.  This disables some of the Micros--t software
// default processing.
...
BOOL CcsEdIdent::OnInitDialog ()
{
	...
	for (ii = 0;true;ii += 1)
	{
		st = CS_csGrpEnum (ii,keyName,sizeof (keyName),description,sizeof (description));
		if (st <= 0) break;
...
void CcsEdIdent::RebuildComboBox ()
{
	...
	for (ii = 0;(st = CS_csEnum (ii,csName,sizeof (csName))) > 0;ii += 1)
	{
...
void CcsEdIdent::DoDataExchange(CDataExchange* pDX)
{
	int st, ii;
	DWORD idx;
	char keyName [32], description [128];
	...
	if (pDX->m_bSaveAndValidate)
	{
		...
		st = CS_csGrpEnum (idx,keyName,sizeof (keyName),NULL,0);
		ASSERT (st == 1);
		m_Group = keyName;
	}
	else
	{
		...
		{
			st = CS_csGrpEnum (ii,keyName,sizeof (keyName),description,sizeof (description));
			if (st <= 0 || !CS_stricmp (keyName,m_Group)) break;
		}
...
void CcsEdIdent::OnSelendokKeyName() 
{
	...
	CS_csEnum (idx,keyName,sizeof (keyName));
...
void CcsEdIdent::OnKillfocusCsidDescr() 
{
	...
	if (wp != NULL)
	{
		wp->GetWindowText (ctemp,sizeof (ctemp));
		m_PropSheet->m_Description = ctemp;
	}
}
/////////////////////////////////////////////////////////////////////////////
...
BOOL CcsEdGnrl::OnInitDialog ()
{
	...
	for (ii = 0;
	     CS_prjEnum (ii,&prjFlags,keyName,sizeof (keyName),description,sizeof (description)) > 0;
		 ii += 1)
	{
	...
	for (ii = 0;
	     CS_quadEnum (ii,description,sizeof (description),&ident) > 0;
	     ii += 1)
	{
...
void CcsEdGnrl::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_StaticLabel (pDX,IDC_CSGN_SELDSCR,m_PropSheet->m_Description);
	...
	{	// Capture possibly modified data.
		...
		st = CS_prjEnum (idx,&prjFlags,keyName,sizeof (keyName),description,sizeof (description));
		ASSERT (st > 0);
		m_PrjKeyName = keyName;
		// Units
		ii = m_UnitCombo.GetCurSel ();
		idx = static_cast<int>(m_UnitCombo.GetItemData (ii));
		st = CS_unEnum (idx,m_PropSheet->m_UnitType,keyName,sizeof (keyName));
		ASSERT (st == 1);
		m_Unit = keyName;
		// Quad
	...
	{	// Display appropriate representation.
		...
		for (ii = 0;true;ii += 1)
		{
			st = CS_prjEnum (ii,&prjFlags,keyName,sizeof (keyName),description,sizeof (description));
			if (st < 1 || !CS_stricmp (keyName,m_PrjKeyName)) break;
		}
		ASSERT (st > 0);
		...
		for (ii = 0;true;ii += 1)
		{
			st = CS_unEnum (ii,m_PropSheet->m_UnitType,keyName,sizeof (keyName));
			if (st < 1 || !CS_stricmp (keyName,m_Unit)) break;
		}
		ASSERT (st == 1);
...
void CcsEdGnrl::OnSelendokCsgnPrjctn() 
{
	...
	st = CS_prjEnum (idx,&prjFlags,keyName,sizeof (keyName),description,sizeof (description));
	ASSERT (st > 0);
	if (!m_PropSheet->NewProjection (keyName))
	{
		char errMesg [256];
		CS_errmsg (errMesg,sizeof (errMesg));
		AfxMessageBox (errMesg);
	...
	if (oldUnits != m_PropSheet->m_UnitType)
	{
		...
		{
			AfxMessageBox ("Projection change required change in type of system unit;\nsystem unit is being set to Meter.");
			m_Unit = "Meter";
		}
		else
		{
			AfxMessageBox ("Projection change required change in type of system unit;\nsystem unit is being set to Degree.");
			m_Unit = "Degree";
		}
	}
	UpdateData (FALSE);
...
void CcsEdGnrl::RebuildUnits (int unitType)
{
	...
	for (ii = 0;
		 CS_unEnum (ii,unitType,keyName,sizeof (keyName)) > 0;
		 ii += 1)
	{
...
void CcsEdGnrl::SetScaleRatio (CString& result,int nIDC,double scale,bool reduction)
{
	...
	else
	{
		...
		else
		{
			...
			CS_ftoa (ctemp,sizeof (ctemp),scale,cs_ATOF_COMMA | 1L);
			result.Format ("1:%s",ctemp);
		}
	}
	if (nIDC != 0)
	{
...
void CcsEdGnrl::OnCsgnElpbtn() 
{
	...
	if (m_RefType [0] == 'D')
	{
		...
		initialKeyName = "WGS84";
		dtPtr = CS_dtdef (m_RefKeyName);
		if (dtPtr != NULL)
		{
			initialKeyName = dtPtr->ell_knm;
			CS_free (dtPtr);
		}
	}
	...
	{
		if (m_RefType [0] != 'E' ||
		    CS_stricmp (m_RefKeyName,elSelect.GetSelectedEllipsoid ()))
		{
			m_PropSheet->OnChange ();
...
void CcsEdGnrl::OnCsgnDatbtn() 
{
	...
	{
		if (m_RefType [0] != 'D' ||
		    CS_stricmp (m_RefKeyName,dtSelect.GetSelectedDatum ()))
		{
			m_PropSheet->OnChange ();
...
void CcsEdGnrl::OnKillfocusCsgnSclred() 
{
	...
	if (cp != NULL)
	{
		*cp++ = '\0';
		...
		sprintf (ctemp,"%.8f",dblTmp);
		wp->SetWindowText (ctemp);
	}
	else
	{
		dblTmp = atof (ctemp);
...
void CcsEdGnrl::OnKillfocusCsgnMapscl() 
{
	...
	if (cp != NULL)
	{
		*cp++ = '\0';
		...
		sprintf (ctemp,"%10.0f",dblTmp);
		wp->SetWindowText (ctemp);
	}
	else
	{
		CS_atof (&dblTmp,ctemp);
	}
...
void CcsEdOrigins::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_StaticLabel (pDX, IDC_CSOR_SELDSCR,m_PropSheet->m_Description);
...
void CcsEdExtents::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_StaticLabel (pDX, IDC_EDCS_SELDSCR,m_PropSheet->m_Description);
...
void CcsEdParms1::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_StaticLabel (pDX,IDC_CSPR_SELDSCR,m_PropSheet->m_Description);
...
{
	int zone;
	char buffer [34];		// _ltoa below assumes at least 34
	...
	if (pDX->m_bSaveAndValidate)
	{
		...
		if (zone < 1 || zone > 60)
		{
			AfxMessageBox ("UTM Zone must be an integer between 1 and 60.");
			pDX->Fail ();
		}
		value = (double)zone;
	}
...
void AFXAPI DDX_HemiNS (CDataExchange* pDX,int nIDC,double& value)
{
	...
	if (pDX->m_bSaveAndValidate)
	{
		...
		len = strlen (buffer);
		if (!CS_strnicmp (buffer,"NORTH",len)) hemisphere = 1;
		if (!CS_strnicmp (buffer,"SOUTH",len)) hemisphere = -1;
		if (hemisphere == 0)
		{
			AfxMessageBox ("Hemisphere must be \"North\" or \"South\" (N or S will also work).");
			pDX->Fail ();
		}
	...
	{
		strcpy (buffer,((int)(value)) >= 0 ? "North" : "South");
		::SetWindowText(hWndCtrl,buffer);
	}
}
...
long AFXAPI DDX_Number (CDataExchange* pDX,int nIDC,double& value,struct cs_Prjprm_ *prmType)
{
	...
	if (pDX->m_bSaveAndValidate)
	{
		...
		else
		{
			...
			{
				errorFlag = true;
				AfxMessageBox ("Value supplied exceeds the maximum value expected for\nthis parameter.");
			}
			else if (tmpDbl < prmType->min_val)
			{
				errorFlag = true;
				AfxMessageBox ("Value supplied is less than the minimum value expected for\nthis parameter.");
			}
		}
		if (errorFlag)
		{
	...
	else
	{
		...
		st = CS_ftoa (buffer,sizeof (buffer),value,prmType->format);
		::SetWindowText(hWndCtrl,buffer);
	}
	return (st);
}
...
void AFXAPI DDX_Scale (CDataExchange* pDX,int nIDC,double& value,bool reduction)
{
	...
	if (pDX->m_bSaveAndValidate)
	{
		...
		if (cp == NULL)
		{
			st = CS_atof (&dblTmp,buffer);
		}
		else
		{
			double nn, dd;
			*cp++ = '\0';
			st = CS_atof (&nn,buffer);
			if (st >= 0)
			{
				st = CS_atof (&dd,cp);
				if (st >= 0)
				{
					...
					{
						errMsg = "Invalid ratio input format. Correct, or use pure decimal form.";
						st = -1;
					}
					else
					{
...
void AFXAPI DDX_CharArray (CDataExchange* pDX,int nIDC,char *data,int count)
{
	...
	{
		::GetWindowText(hWndCtrl,buffer,sizeof (buffer));
		if (strlen (buffer) >= (unsigned)count)
		{
			pDX->Fail ();
		}
