/******************************************************************************
	...
	*********************************************************************  
			 ...
			 File Name: $Source: C:/Development/NewCsMap/rcs/CS_caV2GridFile.c $
			 Full Name:	Version 2 of the Canadian National Grid File
		   Description:
			   Purpose:	Encapulates the purpose and function of a
						datum shift file of the Canadian National
						Transformation, Version2 format.  Includes
						support for the Australian variation. 

		Revision Level:	$Revision: #6 $
				  Date:	$Date: 2006/07/20 $

		 Last Revision:	$Log: CS_caV2GridFile.c $
		 ...
		 Last Revision:	Revision 10.1502  2002/11/13 23:05:34  nto
		 Last Revision:	Added grid data file logging for Nad27ToNad83.
		 Last Revision:	Revision 10.1501  2002/11/06 23:44:39  nto
		 ...
		 Last Revision:	Revision 1.3  2000/01/30 23:24:06  nto
		 Last Revision:	replaced calls to stricmp with CS_stricmp
...
struct csGridFileCa2_* CSnewGridFileCa2 (enum csDtCvtType type,Const char *filePath,long bufferSize,unsigned long flags,double density)
{
	...
	__This->SubOverlap = ((flags & 0x01) != 0);
	__This->ExtType = type;
	...
	malcCnt = sizeof (struct csGridFileCa2SubHdr) * (unsigned long)__This->SubCount;
	...
	for (idx = 0;idx < __This->SubCount;idx += 1)
	{
		...
		subPtr->Cacheable = (__This->IntType == csCaNTv2TypeCanada) && (__This->SubOverlap == 0);
